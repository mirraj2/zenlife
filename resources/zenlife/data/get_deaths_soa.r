# Rscript --slave get_deaths_soa.r --age 20 --term 10 --face 100000 --distribution alb_100_male_nonsmoker_soa
...
age=opt$age
for(j in c(age:(age+25)))
{
	cat(j,file=stdout(),sep="")
	cat(",",file=stdout(),sep="")
	cat(dist2$deaths[j],file=stdout(),sep="\n")
}
quit()